...
apache::mod { 'rewrite': }
apache::vhost { 'local.aiqrcode.com':
    servername => 'local.aiqrcode.com',
    port => '80',
    docroot => '/var/www/AiQRCode/public',
    docroot_owner => 'vagrant',
    docroot_group => 'vagrant',
    directories => [{
        path => '/var/www/AiQRCode/public',
        allow => 'from all',
        allow_override => ['All']
    }]
}
...
class php {
    ...
    exec { 'install php libraries':
        command => 'php -d xdebug.remote_enable=0 composer.phar install --optimize-autoloader -d /var/www/AiQRCode',
